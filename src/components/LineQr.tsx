...
export default function LineQr({ dict, className = "" }: { dict: Dict; className?: string }) {
  return (
    <div className={`items-center gap-4 bg-white rounded-2xl border border-slate-200 p-3 pr-5 text-slate-700 ${className}`}>
      <Image src="/line-qr.svg" alt={`LINE ${LINE_ID}`} width={96} height={96} className="w-24 h-24 shrink-0" unoptimized />
      <div className="text-sm leading-snug">
        <div className="font-bold text-slate-900">{dict.lineExtra.scan}</div>
