    ...
    <section id="guarantee" className="relative overflow-hidden bg-brand text-white section scroll-mt-32">
      ...
      <div className="relative container-x grid lg:grid-cols-[0.9fr_1.1fr] gap-12 items-center">
        <Reveal>
          ...
          <p className="mt-4 text-lg text-white/85">{g.subtitle}</p>
        ...
        <ol className="grid gap-4">
          {g.steps.map((step, i) => (
            <Reveal key={step.title} delay={i * 90}>
              <li className="flex gap-5 rounded-2xl bg-white/10 border border-white/20 p-6">
                <span className="w-10 h-10 shrink-0 rounded-full border border-white/50 text-white font-bold flex items-center justify-center">
                  {i + 1}
                </span>
                <div>
                  <div className="font-semibold text-lg">{step.title}</div>
                  <p className="text-white/80 mt-1 leading-relaxed">{step.body}</p>
                </div>
              </li>
            </Reveal>
          ))}
